...
class OutFile
  ...
  warning_message: """
  /** WARNING: This file is automatically generated by templito.
   *  Do not edit this file if you plan on using templito to continue to
  ...
  constructor: (_path, @options) ->
    ...
    utilities.mkdirp path.basename(@path), =>
      @defaulted_object_paths = []
      @ee.emit 'ready'
      @ready = true
    return null # to supress annoying warning
...
class Template
  ...
  constructor: (_path, @options) ->
    ...
    @path_parts.unshift @options.source_dir
  ...
  get_out_file: ->
    ...
    if fpath == @options.out_dir
      fpath = path.join fpath, path.basename(@options.out_dir)
    new OutFile path.join(@options.out_dir, fpath)
  ...
  compile: (cb) ->
    fs.readFile @path, {encoding: 'utf8'}, (err, source) =>
      ...
      template_path = @path_parts_cased.concat([@name]).join('.')
      # Write to file
